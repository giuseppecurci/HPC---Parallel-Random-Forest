...
void train_forest_1d(Forest *forest, float *data, int num_rows, int num_columns, int num_classes) {
    for (int i = 0; i < forest->num_trees; i++) {
        printf("\rTraining tree %d/%d... (%d%%)", i + 1, forest->num_trees, (i + 1) * 100 / forest->num_trees);
        fflush(stdout);
        train_tree_1d(&forest->trees[i], data, num_rows, num_columns, num_classes, 
                      forest->max_depth, forest->min_samples_split, forest->max_features);
    }
    printf("\n");
}

int* forest_inference_1d(Forest *forest, float *data, int num_rows, int num_columns, int num_classes) {
    ...
    for (int i = 0; i < forest->num_trees; i++) {
        ...
        fflush(stdout);
        predictions_per_tree[i] = tree_inference_1d(&forest->trees[i], data, num_rows, num_columns);
    }
    printf("\n");
