...
int main(int argc, char *argv[]) {
	...
	if (rank == 0) {
		...
		// FIXED: Broadcast all basic parameters first in consistent order
		...
		printf("I am process %d\n", rank);  // FIXED: Added rank to debug output
		
		// FIXED: Receive broadcasts in same order as master sends them
		MPI_Bcast(&test_size, 1, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&num_columns, 1, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&num_classes, 1, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&mode, 1, MPI_INT, 0, MPI_COMM_WORLD);
		
		// Receive sample size and train data via point-to-point communication
		...
		// Allocate memory for targets and test data
		targets = (int *)malloc(test_size * sizeof(int));
		if (!targets) {
		...
		// Receive broadcast data in same order as master
		MPI_Bcast(targets, test_size, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(test_data, test_size * num_columns, MPI_FLOAT, 0, MPI_COMM_WORLD);
	
		if (mode == 0) {
			// NO TREES IN DIRECTORY - slave code
			...
			// Initialize timing variables for all processes
			train_time = 0.0;
			inference_time = 0.0;
			total_time = 0.0;
		...
		// FIXED: Free allocated memory for worker processes
		free(train_data);
		free(test_data);
		free(targets);
	}
	...
	// FIXED: Initialize timing variables for process 0 and handle case where workers might have 0 trees
	...
	// Process 0 reports the maximum times
	if (rank == 0) {
		...
		snprintf(csv_store_time_metrics_path, sizeof(csv_store_time_metrics_path),
				 "output/store_time_metrics_%d_processes.csv", process_number);
